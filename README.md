...
Already implemented: extension surface (Phase X), tool provider injection (Phase Y), advanced architectures (Phase A/B/C), multimodal (Phase F), Kokoro (Phase K), Skynet (Phase S), the **vector store port (Phase Z)**, the **runtime composition root (Phase R)**, the **config source injection (Phase W)**, the **cost & budget governance (Phase C)**, the **runtime hardening (Phase H)**, the **agent evaluation & reliability harness (Phase V)**, the **agent identity & access governance (Phase IDN)**, the **A2A interop (Phase I)**, the **guardrails modernization (Phase GRD)**, and the dependency remediation (18/18 alerts in a terminal state).

New pending phases from the 2026-07 gap analysis (`docs/gap-analysis-loops-harness-guardrails-2026-07.md`), not yet started: **loop hardening** (context compaction + dynamic tool gating, `specs/loop-hardening/`), **node I/O type-safety** (Pydantic validation per graph node, `specs/node-io-typesafety/`), **observability integration** (deep LangSmith/Langfuse parity, `specs/observability-integration/`), and the out-of-repo **reference host bootstrap** (`prismal-server`, `specs/reference-host-bootstrap/`).
...
8. **Polish** — *variable · less urgent* — first-party observability UI / deep LangSmith/Langfuse integration (now specced: [`specs/observability-integration/`](./specs/observability-integration/)) and per-node type safety (now specced: [`specs/node-io-typesafety/`](./specs/node-io-typesafety/)); neither started yet.
