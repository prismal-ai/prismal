...
async def test_eda_analyst_produces_eda_report() -> None:
    ...
    state["metadata"]["ml_pipeline"]["dataset_profile"] = {
        "name": "churn", "path": "tests/fixtures/datasets/churn_sample.csv",
        "rows": 5, "columns": 5, "column_types": {"tenure": "int64"},
        "null_counts": {}, "task_type": "classification", "target_column": "churned",
    }
    with patch("lightagent.providers.registry.ProviderRegistry.get_llm") as mock_llm:
        mock_llm.return_value.ainvoke = AsyncMock(return_value=mock_response)
...
async def test_feature_engineer_produces_feature_set() -> None:
    ...
    state["metadata"]["ml_pipeline"] = {
        "dataset_profile": {
            "name": "churn", "path": "tests/fixtures/datasets/churn_sample.csv",
            "rows": 5, "columns": 5, "column_types": {}, "null_counts": {},
            "task_type": "classification", "target_column": "churned",
        },
        "eda_report": {
            "correlations": {}, "outlier_columns": [],
            "missing_pattern": "MCAR", "class_balance": "imbalanced",
            "recommended_transforms": ["StandardScaler"], "chart_paths": [],
        },
    }
    with patch("lightagent.providers.registry.ProviderRegistry.get_llm") as mock_llm:
        mock_llm.return_value.ainvoke = AsyncMock(return_value=mock_response)
...
async def test_model_trainer_produces_trained_model() -> None:
    ...
    state = _base_state()
    state["metadata"]["ml_pipeline"] = {
        "dataset_profile": {"name": "churn", "task_type": "classification",
                            "target_column": "churned", "rows": 5, "columns": 5,
                            "path": "", "column_types": {}, "null_counts": {}},
        "feature_set": {
...
async def test_model_trainer_enforces_random_seed() -> None:
    ...
    state = _base_state()
    state["metadata"]["ml_pipeline"] = {
        "dataset_profile": {"name": "iris", "task_type": "classification",
                            "target_column": "species", "rows": 6, "columns": 5,
                            "path": "", "column_types": {}, "null_counts": {}},
        "feature_set": {
            "original_features": [], "engineered_features": [],
            "selected_features": [], "encoding_map": {},
            "scaling_method": "none", "train_shape": [4, 4], "test_shape": [2, 4],
        },
    }
    with patch("lightagent.providers.registry.ProviderRegistry.get_llm") as mock_llm:
        mock_llm.return_value.ainvoke = AsyncMock(return_value=mock_response)
...
async def test_model_evaluator_produces_evaluation_report() -> None:
    ...
    state["metadata"]["ml_pipeline"] = {
        "dataset_profile": {"name": "churn", "task_type": "classification",
                            ...
                            "path": "", "column_types": {}, "null_counts": {}},
        "trained_model": {
            "model_type": "LightGBM", "hyperparameters": {},
            "training_time_seconds": 10.0, "framework": "flaml",
            "task": "classification",
            "model_path": "data/workspace/ml_models/churn/model.joblib",
            "random_seed": 42, "validation_score": 0.83,
        },
    }
    with patch("lightagent.providers.registry.ProviderRegistry.get_llm") as mock_llm:
        mock_llm.return_value.ainvoke = AsyncMock(return_value=mock_response)
...
async def test_model_evaluator_low_score_recommends_retrain() -> None:
    ...
    state = _base_state()
    state["metadata"]["ml_pipeline"] = {
        "dataset_profile": {"name": "test", "task_type": "classification",
                            "target_column": "y", "rows": 5, "columns": 2,
                            "path": "", "column_types": {}, "null_counts": {}},
        "trained_model": {
            "model_type": "LogisticRegression", "hyperparameters": {},
            "training_time_seconds": 1.0, "framework": "sklearn",
            "task": "classification",
            "model_path": "data/workspace/ml_models/test/model.joblib",
            "random_seed": 42, "validation_score": 0.50,
        },
    }
    with patch("lightagent.providers.registry.ProviderRegistry.get_llm") as mock_llm:
        mock_llm.return_value.ainvoke = AsyncMock(return_value=mock_response)
...
async def test_model_exporter_produces_model_package() -> None:
    ...
    state["metadata"]["ml_pipeline"] = {
        ...
        "trained_model": {
            ...
            "task": "classification",
            "model_path": "data/workspace/ml_models/churn/model.joblib",
            "random_seed": 42, "validation_score": 0.85,
        },
        "evaluation_report": {
            "metrics": {"f1": 0.85}, "primary_metric": "f1", "primary_score": 0.85,
            "feature_importance": {"tenure": 0.35}, "chart_paths": [],
            "recommendation": "deploy",
        },
    }
    with patch("lightagent.providers.registry.ProviderRegistry.get_llm") as mock_llm:
