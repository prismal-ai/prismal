"""Remote skill installer for Prismal.
...
Supports the same install convention as the Claude Code CLI::
    ...
    # Prismal equivalent
    prismal skills install-remote https://github.com/anthropics/skills \\
        --skill skill-creator

Claude Code skills stored as YAML/Markdown are wrapped in a generated
Prismal-compatible ``BaseSkill`` subclass so they integrate with the
...
logger = structlog.get_logger("prismal.skills.remote_installer")
...
RECOMMENDED_SKILLS: list[dict[str, str]] = [
    {
        ...
        "description": "AI-powered skill generator for Prismal",
    },
]
    ...
    * **Native Python skills** — copied directly (must contain ``skill.py``).
    ...
    async def install(
        ...
        if dest.exists():
            raise ValueError(
                ...
                f"it first with: prismal skills remove {installed_name}"
            )
    ...
    async def _fetch_skill_files(self, repo_slug: str, skill_name: str) -> dict[str, str]:
        ...
        hint = (
            ...
            if skill_names
            else "Run 'prismal skills list-remote <repo>' to see available skills."
        )
    ...
    def _is_claude_code_skill(files: dict[str, str]) -> bool:
        ...
        (which would indicate a native Prismal skill).
    ...
    def _install_claude_code_skill(
        ...
        """Install a Claude Code YAML/MD skill wrapped as a Prismal skill.
    ...
    def _install_native_skill(
        ...
        """Install a native Python-based Prismal skill.
    ...
    def _generate_wrapper_py(
        *,
        ...
        return textwrap.dedent(f'''\
            """Prismal wrapper for the Claude Code skill: {skill_name}.

            Auto-generated by RemoteSkillInstaller
            (prismal.skills.remote_installer).
