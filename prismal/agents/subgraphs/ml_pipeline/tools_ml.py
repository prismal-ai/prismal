"""
ML/DL pipeline tools for Prismal sub-agents.
...
logger = structlog.get_logger("prismal.subgraphs.ml_pipeline.tools")
...
def statistical_summary(path: str, target_column: str = "") -> str:
    ...
    try:
        ...
        return json.dumps({"error": ("polars not installed; run: pip install 'prismal[ml]'")})
    except Exception as exc:
        return json.dumps({"error": str(exc)})
    ...
    try:
        ...
        try:  # pragma: no cover
            ...
            return json.dumps(
                {
                    "status": "unavailable",
                    "error": ("flaml not installed; run: pip install 'prismal[ml]'"),
                    "framework": "flaml",
                    "task": task,
                }
            )
...
def train_dl_model(
    ...
    try:  # pragma: no cover
        ...
        return json.dumps(
            {
                "status": "unavailable",
                "error": ("torch not installed; run: pip install 'prismal[ml-dl]'"),
                "framework": "pytorch",
            }
        )
    except Exception as exc:
...
def train_clustering(
    ...
    try:
        ...
        return json.dumps({"error": ("sklearn not installed; run: pip install 'prismal[ml]'")})
    except Exception as exc:
        return json.dumps({"error": str(exc)})
            ...
            return json.dumps(
                {
                    ...
                    "note": ("install 'prismal[ml]' for SHAP explanations"),
                }
            )
...
def generate_inference_code(
    ...
    script_lines = [
        ...
        "",
        "Generated by Prismal ml_pipeline / generate_inference_code.",
        "Usage:",
