"""Architecture test: Kokoro modules must not import providers at module level.

DD-KOK-004 / RF-KOK-11: every Kokoro component is callable-injected so the
...
Also enforces that no Kokoro module imports ``prismal.mcp`` / ``prismal.skills``
at any nesting level (critical rule 9; the broader agents-wide guard lives in
...
import prismal.agents.kokoro
import prismal.agents.subgraphs.kokoro
import prismal.souls

_PACKAGES = (
    Path(prismal.agents.kokoro.__file__).parent,
    Path(prismal.agents.subgraphs.kokoro.__file__).parent,
    Path(prismal.souls.__file__).parent,
)
...
def test_no_module_level_provider_imports() -> None:
    ...
    assert not offenders, (
        "Module-level provider imports found in Kokoro modules "
        f"(must be lazy/function-level): {offenders}"
    )


def test_no_mcp_or_skills_imports_at_any_level() -> None:
    offenders: list[str] = []
    for file in _module_files():
        tree = ast.parse(file.read_text(encoding="utf-8"))
        for name in _all_imports(tree):
            if name.startswith(_FORBIDDEN_ANYWHERE):
                offenders.append(f"{file}: {name}")
    assert not offenders, f"Forbidden mcp/skills imports in Kokoro modules: {offenders}"
