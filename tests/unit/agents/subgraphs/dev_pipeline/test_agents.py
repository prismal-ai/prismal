...
async def test_architect_agent_adds_tech_spec() -> None:
    ...
    from lightagent.agents.subgraphs.dev_pipeline.architect_agent import architect_agent_node

    state = _base_state()
    state["metadata"]["dev_pipeline"]["user_story"] = {
        "id": "s1", "title": "Login", "description": "Login feature",
...
async def test_developer_agent_adds_code_artifact() -> None:
    ...
    from lightagent.agents.subgraphs.dev_pipeline.developer_agent import developer_agent_node

    state = _base_state()
    state["metadata"]["dev_pipeline"]["technical_spec"] = {
        "id": "spec1", "story_id": "s1", "architecture": "MVC",
        "design_decisions": [], "technology_stack": ["python"]
    }
    mock_response = AIMessage(content=(
...
async def test_unit_test_agent_adds_test_report() -> None:
    ...
    from lightagent.agents.subgraphs.dev_pipeline.unit_test_agent import unit_test_agent_node
...
async def test_reviewer_agent_adds_review_result() -> None:
    ...
    from lightagent.agents.subgraphs.dev_pipeline.reviewer_agent import reviewer_agent_node
