"""ChromaDB vector store adapter for the Prismal RAG system (default backend).

Provides ``ChromaVectorStore``, a thin wrapper around
:class:`~langchain_community.vectorstores.Chroma` that integrates with the
Prismal settings and embeddings systems. It is the default adapter behind
:class:`~prismal.agents.extension.ports.VectorStorePort` and conforms to it
without behaviour change (Phase Z relocation; the public import path
``prismal.rag.vector_store`` still works via a shim).
...
Score contract (SPEC-VS-002): Chroma's ``similarity_search_with_score`` returns
a cosine similarity in ``[0, 1]`` (higher = more relevant). Chroma is the
**reference** for the normalized score contract, so no transformation is applied
here — the value is passed through unchanged.
...
class ChromaStoreError(VectorStoreError):
    ...
    Subclasses :class:`~prismal.core.exceptions.VectorStoreError` (Phase Z) so
    callers can catch any backend's failure uniformly, while existing
    ``except ChromaStoreError`` handlers keep working.
    """
...
class ChromaVectorStore:
    ...
    instance obtained at construction time. Conforms structurally to
    ...
    def similarity_search(
        ...
        Returns:
            ...
            (the SPEC-VS-002 reference; no transformation applied).
