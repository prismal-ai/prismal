...
Status legend: `TODO` · `WIP` · `DONE` · `BLOCKED`. (All rows `DONE` — shipped in v3.3.0.)
...
|---|---|---|---|---|
| V1-01 | `eval/types.py`: `EvalCase`/`EvalSet`/`Assertion`/`Trajectory`/`CaseResult`/`Scorecard` (SPEC-EVL-TYP-001) | 0.6 d | — | DONE |
| V1-02 | `EvalSet.from_yaml` + schema validation; `tests/eval/sets/` layout | 0.4 d | V1-01 | DONE |
| V1-03 | `core/config.py`: `eval_*` settings (SPEC-EVL-CFG-001) + `core/exceptions.py` `EvalError` | 0.3 d | — | DONE |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| V2-01 | `eval/trajectory.py`: `capture_trajectory` from `astream` (+ cost/latency via monitoring) | 0.8 d | V1 | DONE |
| V2-02 | `eval/runner.py`: `EvalRunner.run_case` (per-case `build_test_runtime`, seed) | 0.7 d | V2-01 | DONE |
| V2-03 | `EvalRunner.run_set` → `Scorecard` aggregation | 0.3 d | V2-02 | DONE |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| V3-01 | `eval/assertions.py`: exact / semantic (embeddings port) / tool_usage | 0.6 d | V2 | DONE |
| V3-02 | `eval/judges.py`: `Judge` (LLM-as-judge via `providers/`, metered by Budget) | 0.5 d | V2 | DONE |
| V3-03 | `assert_llm_judge` + `assert_groundedness` (answer ⊂ retrieved context) | 0.5 d | V3-02 | DONE |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| V4-01 | `eval/regression.py`: `compare()` + tolerance; `tests/eval/baselines/` | 0.5 d | V3 | DONE |
| V4-02 | `eval/report.py`: `to_json` / `to_markdown` (+ optional `to_langfuse`) | 0.4 d | V3 | DONE |
| V4-03 | `eval/__main__.py`: `run` / `redteam` / `gate` CLI (SPEC-EVL-CLI-001) | 0.5 d | V4-01,V4-02 | DONE |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| V5-01 | `eval/redteam/`: `load_redteam_corpus` + `assert_security` (containment via audit + OTel) | 0.6 d | V3 | DONE |
| V5-02 | Author corpus: injection (direct+indirect), tool_abuse, exfiltration, jailbreak, prompt_leak (ship `redteam-corpus.example.yaml`) | 0.7 d | V5-01 | DONE |
| V5-03 | Assert containment against L1–L5 (+ Phase H taint/injection/policy when present) | 0.5 d | V5-01 | DONE |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| V6-01 | `pyproject.toml`: `eval` + `redteam` pytest markers; CI job (fakes) | 0.3 d | V4 | DONE |
| V6-02 | CI gate: fail on regression or any passed attack | 0.3 d | V4,V5 | DONE |
| V6-03 | AST guard: `prismal/eval/**` imports only public graph entry + ports (no `agents.*` internals, no `mcp`/`skills`) | 0.3 d | V2 | DONE |
| V6-04 | `docs/eval.md` + `examples/agent_eval.py` + per-release scorecard | 0.5 d | V4 | DONE |
| V6-05 | `README.md` + `CHANGELOG.md`; mark PLAN/SPEC/ARCHITECTURE `IMPLEMENTED` | 0.2 d | V4 | DONE |
