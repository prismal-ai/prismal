...
|---|---|---|---|---|
| S1-01 | `agents/skynet/types.py`: `SwarmOrder`, `SwarmPlan`, `WorkerResult`, `SwarmResult` | 0.4 d | — | TODO |
| S1-02 | `core/exceptions.py`: `SkynetError` hierarchy (SPEC-SKY-ERR-001) | 0.2 d | — | TODO |
| S1-03 | `core/config.py`: `skynet_*` settings (SPEC-SKY-CFG-001) | 0.3 d | — | TODO |
| S1-04 | Settings validation (`skynet_swarm_size ≥ 0`; clamp `skynet_max_swarm` ≤ `parallel_max_workers`) | 0.2 d | S1-03 | TODO |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| S2-01 | `SkynetSupervisor.__init__` with injected `plan_fn` / `evaluate_fn` | 0.4 d | S1 | TODO |
| S2-02 | `plan()`: decompose goal (dynamic vs. fixed size); seed from `unmet` orders | 0.7 d | S2-01 | TODO |
| S2-03 | Cap N at `min(skynet_max_swarm, parallel_max_workers)`; return deferred overflow | 0.4 d | S2-02 | TODO |
| S2-04 | `evaluate()`: `(complete, answer)`; goal/results via `SecurePromptBuilder` | 0.5 d | S2-01 | TODO |
| S2-05 | Default `plan_fn`/`evaluate_fn` lazily wire `ProviderRegistry().get_llm()` | 0.3 d | S2-01 | TODO |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| S3-01 | `SwarmWorker.execute()`: secure prompt, tools via `ToolProviderPort`, gated actions | 0.7 d | S1 | TODO |
| S3-02 | Per-worker failure captured as `WorkerResult(success=False)` (never raised out) | 0.3 d | S3-01 | TODO |
| S3-03 | `reduce_results()`: `synthesis` \| `concat` \| `first_success` | 0.5 d | S1 | TODO |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| S4-01 | State schema with list-append `skynet.results` channel | 0.4 d | S1 | TODO |
| S4-02 | `plan_node`, `worker_node`, `reduce_node`, `evaluate_node`, `output_node` | 0.8 d | S2, S3 | TODO |
| S4-03 | Dispatch conditional edge via `make_parallel_dispatcher(tasks_field, worker_node, max_workers)` | 0.4 d | S4-02 | TODO |
| S4-04 | Re-plan edge: evaluate → plan when not complete and `round < max_rounds` | 0.4 d | S4-02 | TODO |
| S4-05 | `build_skynet_subgraph()` + idempotent `register_skynet()` | 0.3 d | S4-02 | TODO |
| S4-06 | All Skynet state under `state["metadata"]["skynet"]` | 0.2 d | S4-02 | TODO |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| S5-01 | `intent_router.match_intent()` returns `skynet` for swarm/parallel intents | 0.3 d | S4 | TODO |
| S5-02 | `get_async_compiled_graph()` wires `skynet` route when `skynet_enabled` | 0.4 d | S4 | TODO |
| S5-03 | `effective_valid_routes` / `build_system_prompt` gate on the flag | 0.3 d | S5-02 | TODO |
| S5-04 | `DEFAULT_CAPABILITY_MAP["skynet_worker"]` (tools via `ToolProviderPort`) | 0.2 d | S5-02 | TODO |
...
| ID | Task | Estimate | Dependency | Status |
|---|---|---|---|---|
| S6-01 | Unit tests: sizing (dynamic/fixed/cap/defer) | 0.5 d | S2 | TODO |
| S6-02 | Unit tests: worker (secure-prompt + interceptor spies; failure isolation) | 0.4 d | S3 | TODO |
| S6-03 | Unit tests: reduce strategies | 0.3 d | S3 | TODO |
| S6-04 | Unit tests: dispatcher fan-out count + cap + empty/disabled | 0.4 d | S4 | TODO |
| S6-05 | Unit tests: control loop (re-plan, max-rounds, deferred resume) | 0.5 d | S4 | TODO |
| S6-06 | Unit tests: subgraph end-to-end with fakes + no-provider-import guard | 0.5 d | S4 | TODO |
| S6-07 | Integration test: graph snapshot unchanged when `skynet_enabled=False` | 0.3 d | S5 | TODO |
| S6-08 | `docs/skynet.md` + `examples/skynet_swarm.py` | 0.5 d | S5 | TODO |
| S6-09 | `README.md` + `CHANGELOG.md` entries | 0.2 d | S5 | TODO |
...
- [ ] All MUST requirements (RF-SKY-01…RF-SKY-14) implemented and tested.
- [ ] Supervisor-sized swarm runs a parallel order end-to-end and reduces to one result.
- [ ] Fan-out, rounds, and budget caps proven by tests.
- [ ] With `skynet_enabled=False`, zero behavior change (snapshot proven).
- [ ] No provider SDK or `prismal.mcp`/`prismal.skills` import inside `agents/skynet/`.
