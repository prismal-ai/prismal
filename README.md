...
Already implemented: extension surface (Phase X), tool provider injection (Phase Y), advanced architectures (Phase A/B/C), multimodal (Phase F), Kokoro (Phase K), Skynet (Phase S) plus the **Skynet S+ heterogeneous/metered/remote swarms (Phase S+)**, the **vector store port (Phase Z)**, the **runtime composition root (Phase R)**, the **config source injection (Phase W)**, the **cost & budget governance (Phase C)**, the **runtime hardening (Phase H)**, the **agent evaluation & reliability harness (Phase V)**, the **agent identity & access governance (Phase IDN)**, the **A2A interop (Phase I)**, the **guardrails modernization (Phase GRD)**, the **loop hardening (Phase LH)**, the **node I/O type-safety (Phase NTS)**, the **observability integration (Phase OBS)**, the **blind review pipeline (Phase BRP)**, and the dependency remediation (18/18 alerts in a terminal state).

All in-repo SDD phases have shipped. The `prismal-server` reference host is a **separate repository** and out of this repo's scope — it is not tracked in this roadmap.
