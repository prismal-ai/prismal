...
_ROUTING_RULES: list[tuple[list[str], str]] = [
    ...
    (["rsi", "macd", "bollinger", "p/e", "eps", "nvda", "tsla", "aapl",
      "acción", "bursát", "bolsa", "buy", "sell", "hold", "técnico", "fundamental",
      "forex", "cripto", "crypto", "mercado financiero", "stock"], "financial_analyst"),
    # ml_pipeline keywords
    (["entrena", "modelo", "ml", "machine learning", "xgboost", "lightgbm",
      "random forest", "neural", "deep learning", "churn", "clasificaci",
      "regresión", "predicci", "automl", "mlflow", "sklearn", "auc", "f1-score"],
     "ml_pipeline"),
    # dev_pipeline keywords
    (["implementa", "desarrolla", "endpoint", "api", "fastapi", "refactoriza",
      "código", "test", "pytest", "pr ", "pull request", "arquitectura de software",
      "módulo", "clase", "función", "refactor", "bug", "fix", "deploy", "ci/cd",
      "microservicio", "servicio rest"], "dev_pipeline"),
    # data_analyst (default for analytical queries)
    (["analiza", "query", "sql", "dataframe", "dashboard", "kpi", "métrica",
      "usuarios activos", "ventas", "ranking", "segmenta", "calcula el",
      "¿cuántos", "reporte", "estadístic", "distribución", "histograma",
      "correlación", "agrupado", "suma", "promedio", "media", "mediana"],
     "data_analyst"),
]
...
def print_task_header(task: AnalyticsTask) -> None:
    color = AGENT_COLORS[task.domain]
    print(f"\n{'░'*64}")
...
def print_agent_result(result: AgentResult) -> None:
    ...
    for line in summary_lines:
        print(f"     {line}")
    if result.artifacts:
        print(f"  {DIM}  Artefactos: {', '.join(result.artifacts[:3])}"
...
def demo_simulation() -> None:
    ...
    print(f"  6 tareas analíticas → enrutamiento jerárquico determinístico")
    print(f"{'═'*64}")
    ...
    for task in TASKS:
        ...
        verdict = f"{GREEN}✓ correcto{RESET}" if correct else f"{RED}✗ esperado: {task.domain}{RESET}"
...
async def demo_real_langgraph() -> None:
    ...
    print(f"  Usa _make_definition() + MemorySaver, sin LLM real")
    print(f"{'═'*64}")
    ...
    async def demo_analysis_supervisor(state: DemoState) -> dict:
        ...
        if msgs:
            ...
            if getattr(last, "type", "") == "ai" and last_agent != "analysis_supervisor":
                ...
                print(f"  [analysis_supervisor] loop-breaker → END (agente {last_agent} ya respondió)")
                return {"current_agent": "analysis_supervisor", "next_agent": None, "metadata": metadata}
    ...
    print(f"\n{'═'*64}")
    print(f"  Demo LangGraph completado.")
    print(f"  Patrones clave demostrados:")
    print(f"    • Loop-breaker: supervisor detecta respuesta de hoja → END")
    print(f"    • Contador aislado: domain_analysis.iteration_count")
    print(f"    • Router condicional: _analysis_router(state) → str")
    print(f"{'═'*64}")
...
def demo_hierarchy() -> None:
    ...
    print(f"  LIGHTAGENT_HIERARCHICAL_MODE=true")
    print(f"{'═'*64}")
