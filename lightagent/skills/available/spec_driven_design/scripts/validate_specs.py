...
def check_api_spec(filepath: Path) -> list[str]:
    ...
    found_methods = [
        m for m in methods if f"`{m} " in content or ("### " in content and m in content)
    ]
...
def check_data_model(filepath: Path) -> list[str]:
    ...
    if (
        "amount" in content.lower()
        and "Decimal128" not in content
        and "decimal" not in content.lower()
    ):
...
def validate_specs(specs_dir: str) -> int:
